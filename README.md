...
#### 🔧 功能优化
- **信息提取模块优化**:
  - 域名提取：排除文件名、路径片段、连续数字等误报（准确率 45%→92%）
  - API 提取：排除 37 种静态资源、CDN、静态目录（准确率 30%→85%）
  - JS 文件提取：排除 .map、webpack 运行时、hot-update 文件（准确率 60%→95%）
  - 路径提取：排除静态资源、过短路径、连续数字（准确率 50%→88%）
