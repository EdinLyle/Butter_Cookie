/**
 * 又拍云 USS 存储桶检测器
 * @module modules/cloud-storage/detectors/upyun
 */
 ...
 * 又拍云 USS 检测器类
 */
class UpyunDetector extends BaseDetector {
  constructor() {
    super('upyun', '又拍云 USS');
  }
    ...
    for (const targetUrl of candidates) {
      try {
        ...
        if (resp.status === 200 && resp.body) {
          // 又拍云列表响应特征
          const isTraversable =
            (resp.body.includes('<ListBucketResult') || resp.body.includes('<Objects>')) &&
          ...
          if (isTraversable) {
            return this.createVulnerability(
              ...
              '存储桶可公开遍历，可查看桶内文件列表',
              {
                feature: '又拍云 USS 列表响应',
                indicators: ['<ListBucketResult>', '<Objects>', '<Key>'],
              },
            );
          }
        }
        ...
        if (options.debug) {
          console.warn(`[又拍云检测器] 检测可遍历漏洞失败：${error.message}`);
        }
      }
    }
  ...
  async checkUpload(url, options = {}) {
    if (options.safeMode !== false) {
      try {
        ...
        if (options.debug) {
          console.warn(`[又拍云检测器] OPTIONS 检测失败：${error.message}`);
        }
      }
    }
  ...
  async checkACLRead(url, options = {}) {
    ...
    try {
      ...
      if (options.debug) {
        console.warn(`[又拍云检测器] ACL 读取检测失败：${error.message}`);
      }
    }
  ...
  async checkACLWrite(url, options = {}) {
    if (options.safeMode !== false) {
      try {
        ...
        if (options.debug) {
          console.warn(`[又拍云检测器] ACL 写入检测失败：${error.message}`);
        }
      }
    }
  ...
  async checkPolicyRead(url, options = {}) {
    ...
    try {
        ...
        try {
          ...
          if (isPolicyReadable) {
            return this.createVulnerability(
              ...
              '存储桶 Policy 策略可公开读取',
              {
                feature: '又拍云 USS Policy JSON 响应',
      ...
      if (options.debug) {
        console.warn(`[又拍云检测器] Policy 读取检测失败：${error.message}`);
      }
    }
  ...
  async checkPolicyWrite(url, options = {}) {
    if (options.safeMode !== false) {
      try {
        ...
        if (options.debug) {
          console.warn(`[又拍云检测器] Policy 写入检测失败：${error.message}`);
        }
      }
    }
...
// 导出单例
const detector = new UpyunDetector();
 ...
 */
export async function checkUpyun(url, options = {}) {
