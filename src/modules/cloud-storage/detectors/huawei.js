/**
 * 华为云 OBS 存储桶检测器
 * @module modules/cloud-storage/detectors/huawei
 */
 ...
 * 华为云 OBS 检测器类
 */
class HuaweiDetector extends BaseDetector {
  constructor() {
    super('huawei', '华为云 OBS');
  }
    ...
    for (const targetUrl of candidates) {
      try {
        ...
        if (resp.status === 200 && resp.body) {
          // 华为云 OBS 的 ListBucketResult 特征
          const isTraversable =
            resp.body.includes('<ListBucketResult') &&
            resp.body.includes('<Contents>') &&
            (resp.body.includes('<Key>') || resp.body.includes('<Owner>'));
        ...
        if (options.debug) {
          console.warn(`[华为云检测器] 检测可遍历漏洞失败：${error.message}`);
        }
      }
    }
  ...
  async checkUpload(url, options = {}) {
    if (options.safeMode !== false) {
      try {
        ...
        if (options.debug) {
          console.warn(`[华为云检测器] OPTIONS 检测失败：${error.message}`);
        }
      }
    }
  ...
  async checkACLRead(url, options = {}) {
    ...
    try {
      ...
      if (resp.status === 200 && resp.body) {
        ...
        if (isAclReadable) {
          return this.createVulnerability(
            ...
            '存储桶 ACL 配置可公开读取',
            {
              feature: '华为云 OBS ACL XML 响应',
              indicators: ['<AccessControlPolicy>', '<Owner>', '<ID>'],
            },
          );
        }
      }
      ...
      if (options.debug) {
        console.warn(`[华为云检测器] ACL 读取检测失败：${error.message}`);
      }
    }
  ...
  async checkACLWrite(url, options = {}) {
    if (options.safeMode !== false) {
      try {
        ...
        if (options.debug) {
          console.warn(`[华为云检测器] ACL 写入检测失败：${error.message}`);
        }
      }
    }
  ...
  async checkPolicyRead(url, options = {}) {
    ...
    try {
        ...
        try {
          ...
          if (isPolicyReadable) {
            return this.createVulnerability(
              ...
              '存储桶 Policy 策略可公开读取',
              {
                feature: '华为云 OBS Policy JSON 响应',
      ...
      if (options.debug) {
        console.warn(`[华为云检测器] Policy 读取检测失败：${error.message}`);
      }
    }
  ...
  async checkPolicyWrite(url, options = {}) {
    if (options.safeMode !== false) {
      try {
        ...
        if (options.debug) {
          console.warn(`[华为云检测器] Policy 写入检测失败：${error.message}`);
        }
      }
    }
...
// 导出单例
const detector = new HuaweiDetector();
 ...
 */
export async function checkHuawei(url, options = {}) {
