/**
 * 京东云 OSS 存储桶检测器
 * @module modules/cloud-storage/detectors/jd
 */
 ...
 * 京东云 OSS 检测器类
 */
class JDDetector extends BaseDetector {
  constructor() {
    super('jd', '京东云 OSS');
  }
  ...
  async checkTraversable(url, options = {}) {
    ...
    for (const targetUrl of candidates) {
      try {
        ...
        if (options.debug) {
          console.warn(`[京东云检测器] 检测可遍历漏洞失败：${error.message}`);
        }
      }
    }
  ...
  async checkUpload(url, options = {}) {
    if (options.safeMode !== false) {
      try {
        ...
        if (options.debug) {
          console.warn(`[京东云检测器] OPTIONS 检测失败：${error.message}`);
        }
      }
    }
  ...
  async checkACLRead(url, options = {}) {
    ...
    try {
      ...
      if (options.debug) {
        console.warn(`[京东云检测器] ACL 读取检测失败：${error.message}`);
      }
    }
  ...
  async checkACLWrite(url, options = {}) {
    if (options.safeMode !== false) {
      try {
        ...
        if (options.debug) {
          console.warn(`[京东云检测器] ACL 写入检测失败：${error.message}`);
        }
      }
    }
  ...
  async checkPolicyRead(url, options = {}) {
    ...
    try {
        ...
        try {
          ...
          if (isPolicyReadable) {
            return this.createVulnerability(
              ...
              '存储桶 Policy 策略可公开读取',
              {
                feature: '京东云 OSS Policy JSON 响应',
      ...
      if (options.debug) {
        console.warn(`[京东云检测器] Policy 读取检测失败：${error.message}`);
      }
    }
  ...
  async checkPolicyWrite(url, options = {}) {
    if (options.safeMode !== false) {
      try {
        ...
        if (options.debug) {
          console.warn(`[京东云检测器] Policy 写入检测失败：${error.message}`);
        }
      }
    }
 ...
 */
export async function checkJD(url, options = {}) {
